...
use App\Http\Requests\Mini\UserRequest as ThisRequest;
use App\Models\Mini\User as ThisModel;
use App\Transformers\Mini\UserTransformer as ThisTransformer;

class UserController extends Controller { // Mini Program
    ...
    public function index(Request $request, $parent_id=0) {
        ...
        if ( $filter ) $items = $items->where(function($query) use ($filter){
            ...
            return $query->where('slug', 'like', $filter)
